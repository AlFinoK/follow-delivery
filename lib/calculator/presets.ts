...
export interface Preset {
	...
	height: number
	weight: number
	/** legacy: базовая стоимость доставки, ₸. В расчёте больше НЕ участвует (см. CLAUDE.md §10). */
	basePrice: number
	/**
	 * Себестоимость самого товара за единицу, ₸ (ПРАВКИ 2, п.1). Не влияет на цену
	 * доставки: подставляется в позицию накладной и в строку «Стоимость» сводки
	 * логистам. Отдаётся только авторизованным (в публичном калькуляторе = 0).
	 */
	goodsPrice: number
