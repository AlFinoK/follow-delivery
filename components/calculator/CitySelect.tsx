		...
		<div ref={boxRef} className="relative">
			...
			{value ? (
				<button
					type="button"
					onClick={() => setOpen((o) => !o)}
					className="w-full flex items-center gap-2.5 pl-3 pr-2 py-2.5 bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-lg text-left hover:border-slate-300 dark:hover:border-zinc-600 transition-colors">
					<MapPin className="w-4 h-4 text-orange-500 shrink-0" />
					<span className="flex-1 min-w-0 flex items-center gap-2">
						<span className="text-sm font-medium text-slate-900 dark:text-zinc-100 truncate">{value.name}</span>
						{value.approx && (
							<span className="shrink-0 text-[10px] font-medium text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/25 rounded px-1.5 py-0.5">
								{t('calcApproxBadge')}
							</span>
						)}
					</span>
					<span
						...
						className="p-1 text-slate-400 dark:text-zinc-500 hover:text-slate-600 dark:hover:text-zinc-300 rounded">
						<X className="w-4 h-4" />
					</span>
				</button>
			) : (
			...
			{open && !value && (
				<div className="absolute z-30 mt-1.5 w-full max-h-80 overflow-y-auto bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-lg shadow-lg py-1">
					{customExcluded && (
						<div className="flex items-start gap-2 px-3 py-2 text-xs text-red-600 dark:text-red-300">
					...
					{terminals.length > 0 && (
						<>
							<p className="px-3 pt-1.5 pb-1 text-[10px] font-semibold text-slate-400 dark:text-zinc-500 uppercase tracking-wide">
								{t('calcSectionCities')}
							</p>
							{terminals.slice(0, 40).map((d) => (
								<button
									...
									className="w-full flex items-center gap-2.5 px-3 py-2 text-left hover:bg-slate-50 dark:hover:bg-zinc-800/60 transition-colors">
									<Building2 className="w-3.5 h-3.5 text-slate-300 dark:text-zinc-600 shrink-0" />
									<span className="min-w-0 flex-1">
										<span className="block text-sm text-slate-700 dark:text-zinc-200 truncate">{d.name}</span>
										{d.region && d.region !== d.name && (
											<span className="block text-[11px] text-slate-400 dark:text-zinc-500 truncate">{d.region}</span>
										)}
									</span>
								</button>
							))}
					...
					{settlements.length > 0 && (
						<>
							<p className="px-3 pt-2 pb-1 text-[10px] font-semibold text-slate-400 dark:text-zinc-500 uppercase tracking-wide border-t border-slate-100 dark:border-zinc-800 mt-1">
							...
							{settlements.map((p, i) => (
								<button
									...
									className="w-full flex items-center gap-2.5 px-3 py-2 text-left hover:bg-amber-50/60 dark:hover:bg-amber-500/10 transition-colors">
									<Home className="w-3.5 h-3.5 text-amber-400 shrink-0" />
									<span className="min-w-0 flex-1">
										<span className="block text-sm text-slate-700 dark:text-zinc-200 truncate">{p.name}</span>
										<span className="block text-[11px] text-slate-400 dark:text-zinc-500 truncate">{p.region}</span>
									</span>
								</button>
							))}
						</>
					...
					{loading && settlements.length === 0 && (
						<p className="px-3 py-3 text-sm text-slate-400 dark:text-zinc-500 text-center inline-flex items-center justify-center gap-2 w-full">
							<Spinner className="w-4 h-4 text-orange-500" />
							{t('calcSearchingPlaces')}
						</p>
					)}
					{nothing && <p className="px-3 py-3 text-sm text-slate-400 dark:text-zinc-500 text-center">{t('calcNoResults')}</p>}
				</div>
			)}

			{value?.approx && (
				<p className="mt-1.5 text-[11px] text-slate-400 dark:text-zinc-500">{t('calcApproxHint')}</p>
			)}
		</div>
	)
}
