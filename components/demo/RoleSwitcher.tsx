	...
	return (
		<div ref={ref} className="relative">
			<button
				type="button"
				onClick={toggle}
				className={`inline-flex items-center gap-2 bg-white border rounded-lg px-3 py-2 shadow-sm text-sm transition-all ${
					open ? 'border-orange-500 ring-2 ring-orange-500/15' : 'border-slate-200 hover:border-slate-300'
				}`}>
				<UserCog className="w-4 h-4 text-orange-500 shrink-0" />
				<span className="text-xs font-medium text-slate-500 hidden sm:inline">Роль:</span>
				<span className="font-medium text-slate-900">{ROLE_LABELS[role]}</span>
				<ChevronDown className={`w-4 h-4 text-slate-400 shrink-0 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
			</button>
			{open && (
				<div
					className={`absolute ${upward ? 'bottom-full mb-1' : 'top-full mt-1'} right-0 min-w-[190px] bg-white border border-slate-200 rounded-lg shadow-lg z-50 overflow-hidden`}>
					{roles.map((r) => (
						<button
							key={r}
							type="button"
							...
							className={`w-full text-left px-3.5 py-2 text-sm transition-colors flex items-center justify-between gap-2 ${
								r === role ? 'bg-orange-50 text-orange-700 font-semibold' : 'text-slate-700 hover:bg-slate-50'
							}`}>
							{ROLE_LABELS[r]}
