...
export function PresetsPage() {
	...
	const loadPresets = useCallback(async () => {
		setLoading(true)
		try {
			setPresets(await repo.presets.list(true))
		} catch {
	...
	const handleCreate = async (values: PresetFormValues) => {
		setSaving(true)
		try {
			const created: Preset = await repo.presets.create(values)
	...
	const handleUpdate = async (id: string, values: PresetFormValues) => {
		setSaving(true)
		try {
			const updated: Preset = await repo.presets.update(id, values)
	...
	const toggleActive = async (p: Preset) => {
		try {
			const updated: Preset = await repo.presets.update(p.id, { active: !p.active })
	...
	const handleSeed = async () => {
		setSeeding(true)
		try {
			const data = await repo.presets.seed()
